...
contract NatilleraTest is Test {
  ...
  function setUp() public {
    ...
    INatillera.Config memory config =
      INatillera.Config({token: address(token), monthlyContribution: 100 ether, totalMonths: 3, maxMembers: 5});

    INatillera.ProjectInfo memory info = INatillera.ProjectInfo({platform: address(0), projectId: 1, creator: owner});
