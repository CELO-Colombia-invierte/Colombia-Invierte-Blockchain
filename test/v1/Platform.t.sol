...
contract PlatformTest is Test {
  ...
  function setUp() external {
    ...
    platform = new Platform(address(natilleraImpl), address(tokenizacionImpl));
  ...
  function _validNatilleraConfig() internal view returns (INatillera.Config memory) {
    return INatillera.Config({token: address(token), monthlyContribution: 100e18, totalMonths: 3, maxMembers: 5});
  }

  function _validTokenizacionConfig() internal view returns (ITokenizacion.Config memory) {
    return ITokenizacion.Config({
  ...
  function testConstructorSetsOwnerAndImplementations() external view {
    ...
    assertEq(platform.tokenizacionImplementation(), address(tokenizacionImpl));
  }
  ...
  function testDeployNatilleraHappyPath() external {
    ...
    vm.prank(alice);
    address deployed = platform.deployNatillera{value: FEE}(block.timestamp + 1, _validNatilleraConfig());
  ...
  function testDeployNatilleraRefundsExcessETH() external {
    ...
    vm.prank(alice);
    platform.deployNatillera{value: 0.1 ether}(block.timestamp + 1, _validNatilleraConfig());
  ...
  function testDeployTokenizacionHappyPath() external {
    ...
    vm.prank(alice);
    address deployed = platform.deployTokenizacion{value: FEE}(_validTokenizacionConfig());
  ...
  function testTotalProjectsIncrements() external {
    ...
    platform.deployNatillera{value: FEE}(block.timestamp + 1, _validNatilleraConfig());
