...
contract Natillera is Ownable, ReentrancyGuard, INatillera {
   ...
   */
  function initialize(uint256 startTime_, Config calldata config_, ProjectInfo calldata info_) external {
  ...
  function deposit() external onlyMember nonReentrant {
    ...
    IERC20(_config.token).safeTransferFrom(msg.sender, address(this), amount);
  ...
  function hasEnded() public view returns (bool) {
    return block.timestamp >= startTime + (_config.totalMonths * SECONDS_PER_MONTH);
  }
  ...
  function _updateCycle() internal {
    ...
    uint256 cyclesPassed = (block.timestamp - cycleEndTime) / SECONDS_PER_MONTH + 1;
