...
contract Tokenizacion is Ownable, ReentrancyGuard, ITokenizacion {
  ...
  function initialize(Config calldata config_, ProjectInfo calldata info_) external {
    ...
    string memory name = string(abi.encodePacked('Project ', Strings.toString(info_.projectId)));
    string memory symbol = string(abi.encodePacked('PRJ', Strings.toString(info_.projectId)));
  ...
  function buyTokens(uint256 amount) external payable nonReentrant {
    ...
    if (_config.paymentToken == address(0)) {
      ...
      IERC20(_config.paymentToken).safeTransferFrom(msg.sender, address(this), cost);
    }
